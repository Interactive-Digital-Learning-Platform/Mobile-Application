...
import { Award, BookOpen, Brain, CheckCircle2, ChevronDown, Lightbulb, NotebookPen, Sparkles, Target, TriangleAlert } from "lucide-react-native";
      ...
      {insight.items.map((m, i) => {
        ...
        return (
          <View key={m.code} className={i > 0 ? "mt-3 pt-3 border-t border-slate-100" : undefined}>
            <View className="flex-row items-center gap-2">
              <Brain size={14} color={ICON_COLORS.violet600} strokeWidth={2.5} />
              <Text className="text-[13px] font-bold text-slate-800 flex-1">{m.title || m.code}</Text>
              <Text className="text-[11px] font-semibold text-violet-600">{pct}%</Text>
            </View>
            ...
            {!!m.correctionStrategy && (
              <View className="flex-row items-start gap-1.5 mt-1.5 bg-violet-50 rounded-lg p-2">
      ...
      <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
        <Animated.View entering={FadeIn.duration(250)}>
          ...
          {report.misconceptionInsight && report.misconceptionInsight.items.length > 0 && (
            <MisconceptionInsight insight={report.misconceptionInsight} />
          )}
