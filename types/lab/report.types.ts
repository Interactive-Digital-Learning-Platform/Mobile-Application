// --- Lab post-practical report domain types ---
//
// The backend contract (GET /api/sessions/:id/report and /note-context) lives in the first half.
// The second half is the client-side *view model* the redesigned Report screen renders — every
// field there is derived from the contract above by utils/lab/report.ts, never invented.
...
export type ReportCurriculumRefType = {
...
export type LabReportType = {
  ...
  practicalReference?: ReportCurriculumRefType | null;
...
export type LabNoteContextType = {
  ...
  curriculumReference: ReportCurriculumRefType | null;
};

// ── Client view model (derived; see utils/lab/report.ts) ─────────────────────────────────────

export type ReportTabKey = "overview" | "journey" | "improve";

// Visual state for a Main Step in the Lab Journey timeline. Richer than the backend's 4-value
// `status` — derived from hints/help/retries/time so the timeline reads as a journey. Every
// state carries an icon + label (never colour alone).
export type JourneyStatusKey = "strong" | "guided" | "recovered" | "challenging" | "review-time";

export type UnderstandingLevelKey = "needs-support" | "developing" | "proficient" | "strong";

// A compact scan card under the hero. `value` is the headline number/short result.
export type QuickInsightKey =
  | "strongest-step"
  | "challenging-step"
  | "total-hints"
  | "independent-steps"
...
export type JourneyTask = {
  ...
  helpUsed: boolean;
  mistakes: number;
  scoreImpact: number; // hintPenalty + helpPenalty for the task (>= 0)
  advice: string | null;
};
...
export type JourneyStep = {
  ...
  score: number;
  timeSpentSeconds: number;
  status: JourneyStatusKey;
  hintsRequested: number;
  helpUsed: boolean;
  helpRevealedAnswer: boolean;
  retries: number;
  mistakes: number;
  scoreImpact: number;
  isMostChallenging: boolean;
  reasons: string[];
  advice: string | null;
  tasks: JourneyTask[];
};
...
export type LearningMission = {
  key: string;
  category: "concept" | "skill" | "procedure" | "time";
  title: string;
  body: string;
...
export type ReportErrorItem = {
  key: string;
  group: "procedural" | "conceptual" | "behaviour";
  title: string;
  relatedStepLabel: string | null;
  whatHappened: string;
  whyItMatters: string;
  correctiveAction: string;
};

export type TimeBreakdownRow = {
  stepId: number;
  title: string;
  timeSpentSeconds: number;
  ratio: number; // bar fill 0–1 relative to the slowest step
  pace: "fast" | "balanced" | "slow" | null; // null when there's no expected duration
};

export type AITutorHighlights = {
...
export type UnderstandingView = {
  level: UnderstandingLevelKey;
  label: string;
  explanation: string;
  meterFilled: number; // 1–4
} | null;

// Everything the redesigned Report screen needs, computed once from LabReportType.
export type ReportInsights = {
  scoreBand: "high" | "mid" | "low";
  performanceMessage: string;
  understanding: UnderstandingView;
  quickInsights: QuickInsight[];
  strongStepCount: number;
  journey: JourneyStep[];
  guidance: {
    lines: GuidanceLine[];
    hintDeduction: number;
    helpDeduction: number;
    totalDeduction: number;
  } | null;
  challengeInsight: ChallengeInsight | null;
  tutorHighlights: AITutorHighlights;
  tutorSummarySentences: string[];
  achievements: StudentAchievement[];
  missions: LearningMission[];
  errors: ReportErrorItem[];
  timeBreakdown: TimeBreakdownRow[];
};
