...
import Svg, { Path, Ellipse, Line, Defs, ClipPath } from "react-native-svg";
...
export default function TestTubeArt({
  size = 40,
  ...
  // Per-instance clip id — a hardcoded one collides across every container rendered at once.
  const clipId = `tt-interior-${useId()}`;
  ...
  return (
    <Svg height={size} width={(size * VB_W) / VB_H} viewBox={`0 0 ${VB_W} ${VB_H}`}>
      ...
      {/* meniscus / liquid surface line — keeps a colourless fill readable as "there's liquid here" */}
      {liquidColor && level > 0 && (
        <Line x1={6} y1={BOTTOM_Y - liquidHeight} x2={18} y2={BOTTOM_Y - liquidHeight} stroke={color} strokeWidth={1} opacity={0.45} />
      )}
